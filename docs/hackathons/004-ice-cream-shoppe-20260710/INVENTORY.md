# Tool and technology inventory

- **Hackathon Skill** — kickoff structure, milestone rules, evidence capture, and case-study output — `public/hackathon-skill/hackathon-skill.md`
- **Next.js App Router Skill** — persistent layout/provider and interactive component architecture — `vercel:nextjs`
- **Codex filesystem and shell tools** — repository inspection, edits, validation, and Git evidence — Codex workspace tools
- **GPT-5.6** — product strategy, architecture, visual design, implementation, and verification — model under evaluation
- **Playwright + local Chrome** — desktop/mobile rendering, interaction, persistence, hydration, and console verification — bundled Playwright runtime
- **PDF extraction workflow** — rendered and extracted the Apple Notes PDF into a metadata-review staging table — `pdfplumber` + Poppler
- **HEIF conversion runtime** — decoded iPhone HEIC originals into web-sized JPEGs after macOS conversion quality checks — `heif-convert` + `sips`
- **FFmpeg metadata pass** — removed retained EXIF and GPS data from public homemade-pint JPEGs after conversion — `ffmpeg -map_metadata -1`
