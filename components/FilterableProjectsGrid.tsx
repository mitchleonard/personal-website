...
export function FilterableProjectsGrid({ projects }: { projects: Project[] }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {projects.map((cs) => {
        const isHackathon = cs.tags?.includes('Hackathon')
        return (
            ...
            <div className="mb-3">
              <span
                className={`font-sans text-xs font-medium px-3 py-1 rounded-full ${
                  isHackathon ? 'bg-tangerine/25 text-near-black/70' : 'bg-banana/50 text-near-black/70'
                }`}
              >
                {isHackathon ? 'Hackathon' : 'Personal Project'}
              </span>
            </div>
