...
function DraftReview({ draft, approving, onEdit, onApprove }: { draft: DraftPreview; approving: boolean; onEdit: () => void; onApprove: () => void }) {
  ...
  return (
    <section aria-live="polite" className="mt-8 rounded-2xl border border-[#382721]/15 bg-[#fffaf1] p-5 shadow-[0_16px_45px_rgba(56,39,33,.08)] sm:p-7">
      ...
      <p className="mt-3 max-w-xl text-sm leading-6 text-[#382721]/70">This is a private mockup. Approving it publishes the finished entry to the public Shoppe.</p>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        ...
        <button type="button" onClick={onApprove} disabled={approving} className="w-full rounded-full bg-[#d84e72] px-5 py-3 font-bold text-white disabled:opacity-60">{approving ? 'Publishing…' : 'Approve and publish'}</button>
      </div>
    </section>
  )
}
...
export default function ShoppeCounterForm({ ownerId, locations }: { ownerId: string; locations: Location[] }) {
  ...
  async function approveDraft() {
    ...
    setApproving(true)
    setStatus('Publishing your entry to the Shoppe…')
    const now = new Date().toISOString()
    const { error } = await createClient().from('shoppe_submissions').update({ status: 'published', reviewed_at: now, approved_at: now, published_at: now }).eq('id', reviewDraft.id)
    if (error) {
      setStatus(error.message)
    } else {
      setStatus('Published to the Shoppe. No CSV download or re-upload is needed.')
      setReviewDraft(null)
      setFiles([])
  ...
  async function confirmNewLocation() {
    ...
    try {
      const response = await fetch('/api/shoppe/location-search', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...selectedMapResult, displayName: shopQuery.trim() || selectedMapResult.displayName }) })
  ...
  return <>
    <form key={formVersion} ref={formRef} className={reviewDraft ? 'hidden' : 'grid min-w-0 gap-6'} onSubmit={saveDraft}>
      ...
      {kind === 'rating' ? <div className="grid min-w-0 gap-4">
        ...
        <ShoppeDateInput name="made_at" label="Made date" defaultValue={today} />
      ...
      <p className="rounded-xl bg-[#f8ecdf] p-4 text-sm leading-6 text-[#382721]/75">Save first, then review a private mockup. Approving publishes the entry right away. A rating still needs a confirmed storefront; the Shoppe never uses photo GPS as its public map pin.</p>
    </form>
